...
class Predictor:
    ...
    def inference(self, img_path, depth_data, class2track, conf=0.5 ):
        init_sound()
        threat_volume = [1,1]
        origin_img = img_path 
        ...
        if dets is not None:
            ...
            for i in range(len(final_boxes)):
                ...
                if str(self.class_names[cls_id]) == class2track :
                    if centerX <= W/3:
                        W_pos = " left "
            ...
            origin_img = vis(origin_img, final_boxes, final_scores, final_cls_inds, obj_dist, colors_updated, class2track, texts, 
...
def vis(img, boxes, scores, cls_ids, obj_dist, colors, class2track, locations ,conf=0.5, class_names=None):
    #print(colors)
    for i in range(len(boxes)):
        if i < len(colors) and i < len(locations):
            ...
            print(class2track)
            print(class_names[cls_id], cls_id)
            if score < conf:
                continue
            x0 = int(box[0])
            y0 = int(box[1])
